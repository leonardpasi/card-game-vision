...
def find_suits(contours_card):
    ...
    ratio_contours = [
        get_ratio_area_enclosed_area(sorted_contours[i].copy())
    ...
    diffs_lengthes = np.array(
        [
            (
                np.abs(ratio_contours[i + 1] - ratio_contours[i])
                if distance.euclidean(centers[i, :], centers[i + 1, :]) > 600
                else 100
            )
            for i in range(len_contours - 1)
        ]
    )
    ...
    def is_contained_in_suits(
        contour_center, x_interval=x_interval, y_interval=y_interval
    ):
        """
        function that helps identify whether a contour is part of digits or not
        """
        return (
            (contour_center[0] > x_interval[0])
    ...
    contour_digit = [
        sorted_contours[i]
        for i in range(len_contours)
        if is_contained_in_suits(centers[i, :])
    ]
...
def get_contours_one_card(img, ordered_contours_cards, num_player):
    ...
    one_card = cv2.drawContours(
        cimg_copy.copy(),
        ordered_contours_cards.copy(),
        num_player,
        color=255,
        thickness=-1,
    )
    ...
    _, img_cards_only_copy = cv2.threshold(
        greyscale_one_card.copy(), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
    img_cards_only_copy = np.rot90(img_cards_only_copy, k=-num_player)
    contours_card, _ = cv2.findContours(
        img_cards_only_copy, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )

    # In order to omit noise, we put 2 conditions on the contours
    contours_card = [
        contour_
        for contour_ in contours_card
        if (
            cv2.contourArea(contour_) > 800
            and cv2.arcLength(contour_, closed=True) < 2000
        )
    ]
...
def get_needed_contours(final_contours, used_for: str):
    ...
    drawn_contour = cv2.drawContours(
        img_cards_only_copy.copy(), final_contours, -1, color=255, thickness=-1
    )
...
def get_important_contours(
    img, dilate=False, nb_iters=5, hull=False, delay_pixel=0, gamma=1.51
):
    """
    1) Adjust gamma
    ...
    # Get contours
    contours_tmp, _ = cv2.findContours(
        final_im.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )

    if hull:
        contours2 = apply_hull(contours_tmp)
    else:
        contours2 = contours_tmp

    cntsSorted = sorted(contours2, key=lambda x: cv2.arcLength(x, True))[::-1][:5]
    return cntsSorted
...
def get_green_contours(img, delay_pixel=0):
    ...
    mask = cv2.inRange(
        hsv,
        (40 - delay_pixel, 25 - delay_pixel, 25 - delay_pixel),
        (80 + delay_pixel, 255, 255),
    )
...
def get_digits(path, train=False):
    """
    Main function, used to find the digits and return them
    INPUT:
    - path: str: path to the data folder
    - train: Bool: whether we are training (generating data to be able to test it)
    or that we are testing (no labels given)
    """
    X_digits = []
    X_suits = []
    pred_dealer = []
    ...
    if train:
        ...
        for game_nb in range(0, len(games_paths)):
            ...
            results_path = os.path.join(
                path, "train", games_paths[game_nb], "game" + str(game_nb + 1) + ".csv"
            )
            ...
            for round_number in range(len(rounds_paths)):
                ...
                image_paths.append(
                    os.path.join(
                        path, "train", games_paths[game_nb], rounds_paths[round_number]
                    )
                )
        ...
        for round_nb in range(len(rounds_paths)):
            ...
            image_paths.append(os.path.join(path, "test", round))
    ...
    for image_path in image_paths:
        ...
        cntsSorted = get_important_contours(
            img.copy(), dilate=True, nb_iters=5, hull=True
        )

        ratios_area_enclosed_area = np.array(
            [get_ratio_area_enclosed_area(x.copy()) for x in cntsSorted.copy()]
        )
        ...
        if np.all(ratios_area_enclosed_area < 0.7):
            ...
            cntsSorted = get_important_contours(
                img.copy(), dilate=True, hull=True, nb_iters=1, delay_pixel=1
            )
            ratios_area_enclosed_area = np.array(
                [get_ratio_area_enclosed_area(x.copy()) for x in cntsSorted.copy()]
            )
        ...
        if np.any(ratios_area_enclosed_area < 0.5):
            ...
            cntsSorted = get_important_contours(
                img.copy(), dilate=True, nb_iters=7, hull=True, delay_pixel=2
            )
            ratios_area_enclosed_area = np.array(
                [get_ratio_area_enclosed_area(x.copy()) for x in cntsSorted.copy()]
            )
        ...
        center_badge, centers_ordered, ordered_contours_cards = get_centers_and_ordered(
            cntsSorted, center_badge_idx
        )

        centers_distances = np.array(
            [np.linalg.norm(centers_ordered[i, :] - center_badge) for i in range(4)]
        )
        ...
        for iter in range(0, 4):
            contours_card = get_contours_one_card(
                img.copy(), ordered_contours_cards.copy(), iter
            )
            try:
