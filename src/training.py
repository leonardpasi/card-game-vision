...
if ILLUSTRATE:
    ...
    hearts_diamonds = HDClassifier(
        suits_pic[suits_lbl == "H"], suits_pic[suits_lbl == "D"]
    )

    spades_clubs = SCClassifier(
        suits_pic[suits_lbl == "S"], suits_pic[suits_lbl == "C"]
    )
...
id_figures = np.logical_or(
    np.logical_or(digits_lbl == "J", digits_lbl == "Q"), digits_lbl == "K"
)
...
if ILLUSTRATE:
    ...
    for lbl in L:
        ...
        plt.scatter(
            descriptors[digits_lbl == lbl][:, 0],
            descriptors[digits_lbl == lbl][:, 1],
            label=lbl,
            marker=m,
        )
    plt.legend()
